"""pages/Accounts.py - Manage accounts: view balances, create, edit, archive."""
...
st.set_page_config(page_title="Accounts | Wallet Tracker", page_icon="\U0001F3E6", layout="wide")
...
for i, acc in enumerate(accounts):
    with cols[i % 3]:
        st.markdown(
            ...
            f"<div class='metric-value' style='font-size:19px;'>{acc['name']}</div>"
            f"<div style='font-size:20px;font-weight:800;margin-top:6px;'>{format_currency(acc['balance'], acc['currency'])}</div>"
            f"</div>",
